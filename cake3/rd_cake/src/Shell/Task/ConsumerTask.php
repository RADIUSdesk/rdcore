...
class ConsumerTask extends Shell{
    ...
    public function singleMeshReport($report){
     ...
     public function _new_report(){
        ...
        if($entity){
            ...
            $r0      = $this->data['network_info']['radios'][0]['interfaces'];  
            $this->_do_radio_interfaces($entity,$r0);

            //If it is a dual radio --- report on it also ----
            if (array_key_exists(1, $this->data['network_info']['radios'])) {
                $r1 = $this->data['network_info']['radios'][1]['interfaces'];
                $this->_do_radio_interfaces($entity,$r1);
            }
        
            //Update the Loads
            $this->_do_node_load($entity->id,$this->data['system_info']['sys']);
            //Update system info (if required)
            $this->_do_node_system_info($entity->id,$this->data['system_info']['sys']);
            //Spider web   
            $this->_do_vis();
        }
        return $items;
    ...
    private function _do_mesh_mac_lookup(){
        ...
        $e_lookup = $this->{'Nodes'}->find()->where(['mesh_id' => $this->meshId])->all();
    ...
    private function _do_uptm_history($entity){
        ...
        $bootcycle      = 0;
        $drift          = 0;
        if (array_key_exists('bootcycle', $this->data)) {
            $bootcycle  = intval($this->data['bootcycle']); 
        }else{
            return; 
        }
        
        if (array_key_exists('drift', $this->data)) {
            //FIXME Leave for now
           // $drift  = intval($this->data['drift']); 
        }
    ...
    private function _do_vis(){
        ...
        if (array_key_exists('vis', $this->data)) {     
            foreach ($this->data['vis'] as $vis) {
                $metric         = $vis['metric'];
