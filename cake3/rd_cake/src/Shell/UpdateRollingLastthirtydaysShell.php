...
// 35 * * * * www-data cd /var/www/radiusdesk/cake3/rd_cake && bin/cake update_rolling_lastthirtydays >> /dev/null 2>&1
...
// call sp_update_rolling_last_thirty_days(@in_date_time,@in_date,@dead_seconds,@nms,@ins_err_cnt,@err_update_cnt);
...
class UpdateRollingLastthirtydaysShell extends Shell{
    public function main(){
        $this->out("Updating rolling last thirty days");
        ...
        $this->connection->execute("call sp_update_rolling_last_thirty_days(@in_date_time,@in_date,@dead_seconds,@nms,@ins_err_cnt,@err_update_cnt)");
