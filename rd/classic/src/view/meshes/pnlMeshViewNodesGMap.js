Ext.define('Rd.view.meshes.pnlMeshViewNodesGMap', {
    extend: 'Ext.ux.GMapPanel',
    alias: 'widget.pnlMeshViewNodesGMap',
    markers: [],
    infoWindows: [],
    polyLines: [],
    ...
    addMarker: function (marker) {
        ...
        o.setMap(me.gmap);
        me.bounds.extend(o.getPosition())
        me.markers.push(o);
        return o;
    },
    ...
    populateMarkers: function (markers) {
        if (markers.length > 0) {
            ...
            Ext.Array.forEach(markers, function (marker) {
                ...
                marker.position = new google.maps.LatLng(parseFloat(marker.lat), parseFloat(marker.lon));

                var o = new google.maps.Marker(marker);
                o.setMap(self.gmap);

                self.bounds.extend(o.getPosition())

            });
            self.gmap.setCenter(new google.maps.LatLng(markers[0].lat, markers[0].lon));
        }

    },
    populatePolylines: function (lines) {
        var me = this;

        Ext.Array.forEach(lines, function (line) {
            me.addPolyLine(line);
        })

    },
    onMaprender: function (map, gmap, other) {
        //this.gmap = new google.maps.Map(map);
    },
    onDivLoaded: function () {
        var me = this;
        setTimeout(function () {

           // me.populateMarkers(me.markers);
          //  me.populatePolylines(me.polyLines);
        }, 500)
